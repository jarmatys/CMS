    ...
    public class HomeService : IHomeService
    {
        ...
        public async Task<HomeView> GetHomeProperties()
        {
            var homeView = new HomeView
            {
                ...
                Articles = Enumerable.Reverse(await _context.Articles.Where(x => x.IsDraft != true).Include(x => x.Image).Include(x => x.User).Include(x => x.Comments).ToListAsync()).Take(3).OrderByDescending(a => a.AddDate),
